...
*/
		...
		// if no cache file or cache expired, load the data via curl and save in the cache
		if(!file_exists($file) || filemtime($file) < time() - 3600) 
		{
	        $ch = curl_init('http://xml.weather.yahoo.com/forecastrss?p=' . urlencode($zip) . '&u=' . urlencode($units['temp']));
		...
		// regardless of whether fresh or cached, load the data for parsing
		$this->data = @file_get_contents($file);
